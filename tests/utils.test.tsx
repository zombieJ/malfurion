...
describe('Utils', () => {
  describe('Matrix', () => {
    ...
    it('fromTransform', () => {
      ...
      expect(m.getMatrix()).toEqual([[1, 3, 5], [2, 4, 6], [0, 0, 1]]);
    });
    ...
    it('multiple', () => {
      ...
      expect(result.getMatrix()).toEqual([[22, 28], [49, 64]]);
    });
  ...
  describe('Line', () => {
    it('fromPoints', () => {
      expect(Line.fromPoints({ x: 1, y: 2 }, { x: 1, y: 3 }).toUnits()).toEqual(
        [null, null, 1, null],
      );

      expect(Line.fromPoints({ x: 1, y: 2 }, { x: 3, y: 2 }).toUnits()).toEqual(
        [null, null, null, 2],
      );

      expect(Line.fromPoints({ x: 1, y: 2 }, { x: 2, y: 3 }).toUnits()).toEqual(
        [1, 1, null, null],
      );
    });
    ...
    it('getPerpendicular', () => {
      ...
      expect(verticalLine.getPerpendicular(point).toUnits()).toEqual([
        null,
        null,
        null,
        2,
      ]);
      expect(horizontalLine.getPerpendicular(point).toUnits()).toEqual([
        null,
        null,
        -2,
        null,
      ]);
      expect(line1.getPerpendicular(point).toUnits()).toEqual([
        -0.5,
        1,
        null,
        null,
